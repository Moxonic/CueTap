...
// passes while a real search fails. Hard-wired to 10 so no caller can trip that.
export async function searchTracks(query: string): Promise<TrackHit[]> {
  if (!query.trim()) return []
  const res = await call(`/search?type=track&limit=10&q=${encodeURIComponent(query)}`)
